-- Fix FK violation in reconcile_duplicate_persons: the FOR loop
-- pre-fetches all pairs, so a pid deleted as secondary in pair N
-- may appear as primary in pair N+1. Guard each iteration by checking
-- that both pids still exist in persons before merging.
...
BEGIN
    ...
    FOR rec IN
        ...
        LIMIT max_pairs
    LOOP
        -- Guard: skip if either pid was deleted by a previous merge
        -- within this same batch.
        IF NOT EXISTS (
            SELECT 1 FROM localize.persons WHERE person_record_id = rec.pid1
        ) THEN
            INSERT INTO touched_pids (pid) VALUES (rec.pid1) ON CONFLICT DO NOTHING;
            CONTINUE;
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM localize.persons WHERE person_record_id = rec.pid2
        ) THEN
            INSERT INTO touched_pids (pid) VALUES (rec.pid2) ON CONFLICT DO NOTHING;
            CONTINUE;
        END IF;
